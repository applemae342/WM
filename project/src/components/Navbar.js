    ...
    return (
        <nav className="fixed top-0 w-full bg-gray-50 shadow-md z-50 font-sans">
            <div className="flex items-center justify-start h-16 px-0">
                {" "}
                {/* Removed padding for left alignment */}
                <Link href="/" passHref>
                ...
                <div className="hidden md:flex space-x-7 ml-auto mr-8">
                    ...
                    <Link href="#home" passHref>
                        <span className={`cursor-pointer ${activeLink === "home" ? "text-blue-500" : "text-black"} text-lg`}>{homeTitle}</span>
                    </Link>
                    <Link href="#about" passHref>
                        <span className={`cursor-pointer ${activeLink === "about" ? "text-blue-500" : "text-black"} text-lg`}>{aboutTitle}</span>
                    </Link>
                    <Link href="#contact" passHref>
                        <span className={`cursor-pointer ${activeLink === "contact" ? "text-blue-500" : "text-black"} text-lg`}>{contactTitle}</span>
                    </Link>
