...
const AddressConfig = ({ onBackClick2 }) => {
    ...
    const handleFinishClick = async () => {
        ...
        try {
            ...
            console.error("Registration failed:", error.response ? error.response.data : error.message); // Log error details
            ...
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="bg-white p-6 rounded-lg shadow-lg">
                        ...
                        <a href="/sign_in_page">
                            <button onClick={() => setShowModal(false)} className="mt-4 bg-blue-500 text-white font-semibold py-2 px-4 rounded-md">
                                Close
                            </button>
                        </a>
                    </div>
