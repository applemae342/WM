...
const SignIn = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const apiUrl = isTruckFormVisible ? "http://localhost:8000/API/loginTruck/login" : "http://localhost:8000/API/login";

            const response = await axios.post(apiUrl, { username, password });

            if (response.status === 200) {
                if (isTruckFormVisible) {
                    const { truckId, plateNumber, username: truckUsername, description, password: truckPassword, routesID } = response.data.truck;
                ...
                <div className="bg-white rounded-md shadow-lg p-8 w-full max-w-xl">
                    ...
                    {!isTruckFormVisible && (
                        <div className="text-center mb-4">
                            <button onClick={() => setIsTruckFormVisible(true)} className="text-blue-600 hover:underline">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div>
                                <p>
                                    ...
                                    <Link href="/forgot_password" className="text-blue-500"> here</Link>.
                                </p>
                                </div>
                    ...
                    {isTruckFormVisible && (
                        <>
                            ...
                            <div className="text-center mt-4">
                                <button onClick={() => setIsTruckFormVisible(false)} className="text-blue-600 hover:underline">
                                    Back to User Login
                                </button>
                            </div>
                        </>
