...
const ResidentsDashboard = () => {
    ...
    return (
        <div className="flex h-screen font-sans">
            {" "}
            {/* Apply Open Sans to the entire dashboard */}
                ...
                <div className="flex flex-col h-full">
                    ...
                    <div className="mt-2 flex-grow">
                        <ul className="space-y-4">
                            ...
                            <li
                                className={`flex items-center cursor-pointer p-2 rounded-md transition duration-200 ${view === "locationTracking" ? "bg-gray-600" : "hover:bg-gray-600"}`}
                                onClick={() => setView("locationTracking")}
                            >
                                <Image src="/images/location.png" alt="Location Tracking" width={37} height={37} className="mr-2 ml-3" />{" "}
                                {/* Location Tracking image */}
                                {!isCollapsed && <span className="text-lg ml-2 font-sans">Location Tracking</span>} {/* Ensure font is Open Sans */}
                            </li>
                            <li
                                className={`flex items-center cursor-pointer p-2 rounded-md transition duration-200 ${view === "reviews" ? "bg-gray-600" : "hover:bg-gray-600"}`}
                                onClick={() => setView("reviews")}
                            >
                                <Image src="/images/announcement.png" alt="Reviews" width={29} height={29} className="mr-2 ml-4" />{" "}
                                {/* Reviews image */}
